    ...
    public float[] bindRect(ViewGroup parent, Matrix matrix, boolean force){
        if (srcPoints == null || force){
            if (parent.getChildCount() > 0) {
                ...
                srcPoints = new float[]{child.getLeft() - padding, child.getTop() - padding,
                        child.getRight() + padding, child.getTop() - padding,
                        child.getRight() + padding, child.getBottom() + padding,
                        child.getLeft() - padding, child.getBottom() + padding};
                rect = new Rect(child.getLeft() - padding, child.getTop() - padding, child.getRight() + padding, child.getBottom() + padding);
       ...
       // Logger.d("bindRect... x= "+ center[0] +"  ,y= "+ center[1] +"  ,sx= "+ scale[0] +"  ,sy= "+ scale[1] +" ,degree= "+ degree);
        ...
        if (Math.abs(scale[0]) >= minScale[0] && scale[1] >= minScale[1]){
            ...
            matrixFix.postTranslate(center[0] - (rect.right - rect.left) /2 + padding * 1,center[1] - (rect.bottom - rect.top)/2 + padding * 1);
            //matrixFix.postTranslate(center[0],center[1]);
...
//        canvas.drawPath(path, paint);
        ...
        if (isZoom){
            ...
            if ((fixPoints[positionX] < -zoom.getWidth() && fixPoints[positionY] < -zoom.getHeight()) ||
                    (fixPoints[positionX] > width + zoom.getWidth() && fixPoints[positionY] < height + zoom.getHeight())){

            }else{
                saveCount = canvas.save();
                canvas.rotate(degree, fixPoints[positionX], fixPoints[positionY]);
                canvas.drawBitmap(zoom, fixPoints[positionX] - zoom.getWidth()/2, fixPoints[positionY] - zoom.getHeight()/2, bitmapPaint);
                canvas.restoreToCount(saveCount);
            }
        }
        if (isClose){
            ...
            if ((fixPoints[positionX] < -close.getWidth() && fixPoints[positionY] < -close.getHeight()) ||
                    (fixPoints[positionX] > width + close.getWidth() && fixPoints[positionY] < height + close.getHeight())){

            }else{
                canvas.drawBitmap(close, fixPoints[positionX] - close.getWidth()/2, fixPoints[positionY] - close.getHeight()/2, bitmapPaint);
            }
        }
    }
